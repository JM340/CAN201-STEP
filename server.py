...
import uuid
import math
from socket import * #Debug: add socket 
import shutil
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_UPLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is True:
            ...
            connection_socket.send(
            #Debug:408->402
                make_response_packet(OP_UPLOAD, 402, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is completely uploaded.', {}))
            return
...
def Step_service(connection_socket, addr):
    ...
    while True:
        ...
        logger.warning(json_data.keys())
        logger.warning(json_data.values())
